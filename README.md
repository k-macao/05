...
## 逐条 AI 分析（ai_analyzer.py）

每条抓取到的新闻标题均自动附加 AI 分析标签，包含：

- **利好 / 利空 / 中性** + 涉及的板块（如：半导体、光通信、医药/生物、原油/能源、外汇等）
- **涉及市场**（A股 / 港股 / 美股 / 商品期货 / 外汇 / 债券 / 多市场）
- **影响周期**（短期 / 中期 / 长期）

分析结果体现在：
1. **PushPlus 推送 HTML**：每条新闻标题下方显示彩色标签（绿色利好 / 红色利空 / 紫色中性 + 蓝色市场 + 橙色周期）
2. **API 响应**：`/api/brief` 返回的每条 item 包含 `analysis` 字段

当前为纯规则引擎（关键词词典 + 语义加权），零第三方依赖，后续可替换为 LLM API 调用。
