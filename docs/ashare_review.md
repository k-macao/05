...
> 位置：开篇 AI 部分。「AI 每日总结」四个观点之后，紧跟「AI 看盘」板块（其后才是「AI 情绪热力图 → AI 政策分析 → AI 政策深度 → AI 政策研报 → AI 板块机会 → 全网快讯」）。
...
与 49 个新闻源一致：**实时采集（主源 → 备用源）→ 内置快照兜底**，保证任何环境（本地 / GitHub Pages / GitHub Actions）都能稳定出内容。
