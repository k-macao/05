...
| 规则 | 说明 | 实现 |
| --- | --- | --- |
| **每源 3 条** | 18 个数据源，每个源按抓取顺序取前 3 条，最多 54 行 | `NEWS_ITEMS_PER_SOURCE = 3`，`_news_card(per_source)` |
...
- 默认（会员 10 万字 / 98,000 字符）：54 行全部列出，实测整份推送约 1.8 万字符，余量充足。
- `PUSHPLUS_MEMBER=0`（普通账号 2 万字 / 19,500 字符）：同样按每源 3 条输出，
  只有当分析栏目本身就把额度吃满时才会触发收敛。
