...
> 更新时间：**2026-09-19**（按五大板块分组；48 个数据源）
...
| 规则 | 说明 | 实现 |
| --- | --- | --- |
| **每源 3 条** | 48 个数据源（五大板块），每个源按抓取顺序取前 3 条，最多 144 行 | `NEWS_ITEMS_PER_SOURCE = 3`，`_news_card(per_source)` |
...
- 默认（会员 10 万字 / 98,000 字符）：144 行全部列出，演示数据实测整份推送约 2.8 万字符，余量充足。
- `PUSHPLUS_MEMBER=0`（普通账号 2 万字 / 19,500 字符）：48 源 × 3 条放不下，自动收敛到每源 1 条
