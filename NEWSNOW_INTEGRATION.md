...
> 历史文档（2026-09 初）：记录 12 → 18 源的接入过程。之后又按同一套「元信息 + 收集器 + 演示兜底」模式
> 加入了「政策发布 · 官方信息源」「全球政经媒体」「公民科技 · 政治透明度」三个板块，总数扩展到 **49 个源**，
> 当前板块与源清单以 `sources.SECTIONS` / `SOURCE_META` 和 `docs/policy_sections.md` 为准。
